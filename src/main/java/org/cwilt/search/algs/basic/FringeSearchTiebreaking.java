...
public class FringeSearchTiebreaking extends org.cwilt.search.search.SearchAlgorithm {
	
//	protected final HashMap<Object, SearchState> astarNodes = new HashMap<Object, SearchState>();
	protected int skips;
	protected int iterations;
	private final Comparator<SearchNode> c = new SearchNode.FGComparator();
	
	public FringeSearchTiebreaking(SearchProblem prob, Limit l) {
		super(prob, l);
		this.open = new FringeOpen<SearchNode>();
		this.closed = new HashMap<Object, SearchNode>();
		
		fLimit = SearchNode.makeInitial(prob.getInitial());
...
//		}
	...
	protected SearchNode minRejectedF;
	protected SearchNode fLimit;
...
//					assert(!onPath);
					addRejected(child);
					if(minRejectedF == null || c.compare(child, minRejectedF) < 0)
						minRejectedF = child;
				} else {
					openIter.add(child);
				}
	...
	protected boolean keepGoing() {
		return l.keepGoing()
				&& (getIncumbent() == null || (getIncumbent().getCost() > fLimit.getF()));
	}
...
//			System.err.println("fLimit " + fLimit + " at " + l.getDuration());
			...
			while (nIter.hasNext() && keepGoing()) {
				...
				if(c.compare(n, fLimit) > 0){
					if(minRejectedF == null || c.compare(n, minRejectedF) < 0)
						minRejectedF = n;
					skips ++;
					continue;
				}
			...
			if (getIncumbent() == null || getIncumbent().getCost() > fLimit.getF()) {
