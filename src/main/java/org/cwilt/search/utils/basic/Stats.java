...
public class Stats {
	public static double pearson(ArrayList<Double>a1, ArrayList<Double> a2){
		assert(a1.size() == a2.size());
		
		double a1Mean = mean(a1);
		double a2Mean = mean(a2);
		double sx = 0;
		double sy = 0;
		double sxy = 0;
		
		for(int i = 0; i < a1.size(); i++){
			double xErr = a1.get(i) - a1Mean;
			double yErr = a2.get(i) - a2Mean;
			sx += xErr * xErr;
			sy += yErr * yErr;
			sxy += xErr * yErr;
		}
		
		return Math.sqrt(sxy * sxy / (sx * sy));
	}
	...
	public static double stdev(double[] a){
		...
		double sse = 0;
		for(double v : a){
			sse += Math.pow(mean - v, 2);
		}
		return Math.sqrt(sse / a.length);
	}
	public static double stdev(ArrayList<Double> a) {
		double mean = mean(a);
		double sse = 0;
		for(double v : a){
			sse += Math.pow(mean - v, 2);
		}
		return Math.sqrt(sse / a.size());
	}
